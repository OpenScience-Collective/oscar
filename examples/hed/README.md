...
1. **One `llms.txt` at the site root** (Optional tier: cheap and honest, with no measured uptake) that maps every page that matters, the introduction, the resources hub,
...
3. **One site-level [`AGENTS.md`](after/AGENTS.md)** (Optional at a site root; the Core placement is a repo root) that states, in plain text, what HED is and how to validate,
   so an agent editing a dataset knows to check its work rather than guess.

For pages with substantial prose (the resources page, the tutorials), optionally add a markdown mirror,
...
- **After:** the JSON-LD and an AI-aware `robots.txt` (the Core pair), plus the `llms.txt` and the `AGENTS.md`
...
- **Website** ([guide](../../docs/archetypes/website.md)): server-rendered JSON-LD first, then the `llms.txt` and markdown mirrors.
